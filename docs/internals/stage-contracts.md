...
| `parse` | `parse` | Baseline or update XML selected by `--source` | One parsed shard per input under `baseline_jsonl/` or `update_jsonl/` | Same as `parse-one`, with one manifest row per input file | Per-file complete-output skip |
| `resolve-state` | `resolve-pubmed-state` | Parsed baseline and update streams | Live records under `data/temp_data/pubmed/resolved_jsonl/` plus state counts; parsed input shards are removed after success | Applies updates and deletions in source and record order; records `cleaned_baseline_shards` and `cleaned_update_shards` | Rebuilds deterministic resolved outputs; resolution failure leaves parsed inputs intact |
| `validate` | `validate-json` | Resolved `.jsonl` or `.json` file or directory | Console validation report only | `records` is valid records read before errors; `deleted` is failure count; metadata records file count and failures | No output to resume |
...
| `transform` | `transform` | Resolved input file or all files in `data/temp_data/pubmed/resolved_jsonl/` | One article shard per input in `data/temp_data/article_parquet/` | Same as `transform-one`; one-output-per-input mode | Per-output complete-file skip |
...
| `transform-shards` | `transform-shard` | All resolved JSONL files in `data/temp_data/pubmed/resolved_jsonl/` | Atomic `transform_inputs.txt` and one output per expected shard, including empty canonical shards | One manifest row per shard | Rewrites input list; each shard applies its own resume check |
