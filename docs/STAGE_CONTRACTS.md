...
| `external-all` | individual `external-*` rows | All configured external raw inputs | All configured external processed outputs | One manifest row per external preprocessor | Each preprocessor applies its own resume check |
...
Skipped rows should include metadata explaining the skip reason. Failure rows should include `error` text where the command catches the exception.
